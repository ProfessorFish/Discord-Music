...
module.exports = {
  ...
  slashData: new SlashCommandBuilder()
    .setName('queue')
    .setDescription("Get the server's queue."),
  ...
  slash: async function(interaction, client) {
    let thisPlayer = client.players.find(k => k.id === interaction.guild.id)
    if (!thisPlayer) return interaction.reply("No music playing!")
    var thisQueue = thisPlayer.data.queue;
    var thisSongs = thisQueue.songs;
    let amountOfPages = Math.ceil(thisSongs.length / 10)
    var pages = []
    ...
    client.functions.slashButtonFlick(interaction, pages, {
      timeout: 60 * 1000,
      buttons: {
        delete: {
          style: "DANGER",
